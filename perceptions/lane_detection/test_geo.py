...
class TestWithinCone(unittest.TestCase):
    def test_within_cone_simple(self):
        from .geo import within_cone
        
        car_pos = np.array([0.0, 0.0])
        # Heading 0 radians (pointing right along X axis)
        heading = 0.0
        cone_angle = 120 * np.pi / 180  # 120 degrees
        
        # 1. Point directly ahead (angle 0) - Should PASS
        p_ahead = np.array([10.0, 0.0])
        self.assertTrue(within_cone(p_ahead, car_pos, heading, cone_angle), 
                        "Point directly ahead should be in cone")
                        
        # 2. Point at 45 degrees (inside 60 deg half-angle) - Should PASS
        p_45 = np.array([10.0, 10.0]) 
        self.assertTrue(within_cone(p_45, car_pos, heading, cone_angle),
                        "Point at 45 deg should be in cone")
                        
        # 3. Point at 90 degrees (outside 60 deg half-angle) - Should FAIL
        p_90 = np.array([0.0, 10.0])
        self.assertFalse(within_cone(p_90, car_pos, heading, cone_angle),
                         "Point at 90 deg should NOT be in cone")
                         
        # 4. Point slightly behind (angle > 90) - Should FAIL
        p_behind = np.array([-1.0, 10.0])
        self.assertFalse(within_cone(p_behind, car_pos, heading, cone_angle),
                         "Point behind should NOT be in cone")
                         
        # 5. Point directly behind - Should FAIL
        p_back = np.array([-10.0, 0.0])
        self.assertFalse(within_cone(p_back, car_pos, heading, cone_angle),
                         "Point directly behind should NOT be in cone")

    def test_within_cone_rotated(self):
        from .geo import within_cone
        
        car_pos = np.array([0.0, 0.0])
        # Heading 90 degrees (pointing up along Y axis)
        heading = np.pi / 2
        cone_angle = 120 * np.pi / 180
        
        # 1. Point directly ahead (at Y=10) - Should PASS
        p_ahead = np.array([0.0, 10.0])
        self.assertTrue(within_cone(p_ahead, car_pos, heading, cone_angle),
                        "Rotated: Point directly ahead should be in cone")
                        
        # 2. Point to the right (at X=10) -> -90 deg relative to heading -> Outside - Should FAIL
        p_right = np.array([10.0, 0.0])
        self.assertFalse(within_cone(p_right, car_pos, heading, cone_angle),
                         "Rotated: Point at -90 deg relative should NOT be in cone")
