...
def augment_map(
    cone_map: Map, false_positive_rate: float = 0.1, dilatation: float = 0.2
) -> Map:
    """
    ...
    false_points = np.random.uniform(
        low=expanded_min, high=expanded_max, size=(num_false_points, 2)
    )
...
def generate_perceptual_field_data(
    ...
    # Validating augment variables are not present in scope, removing dead code
    # if augment:
