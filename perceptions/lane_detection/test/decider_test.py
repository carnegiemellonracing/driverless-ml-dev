...
from perceptions.lane_detection.deciders import next_vertex_decider, left_right_decider
...
class TestLeftRightDecider(unittest.TestCase):
    ...
    def test_insufficient_lane_history(self):
        ...
        result = left_right_decider(ctx, left_lane, right_lane, left_candidate, right_candidate)
        self.assertEqual(result, 0, "Should default to left (0) with insufficient history")
    ...
    def test_straight_lane_prefer_left_smooth(self):
        ...
        result = left_right_decider(ctx, left_lane, right_lane, left_candidate, right_candidate)
        self.assertIn(result, [0, 1], "Result should be 0 (left) or 1 (right) for straight case")
