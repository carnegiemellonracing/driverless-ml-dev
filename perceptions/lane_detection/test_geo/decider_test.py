...
class RealTrackDeciderTester:
    ...
    def test_track(self, track_id, num_points=10):
        ...
        right_path = [
            i + len(track["left_points"])
            for i in range(min(num_points, len(track["right_points"])))
        ]
            ...
            for violation_type, violation_data in bt_result["violations"]:
                ...
                if violation_type == "seg_angle" and violation_data:
                    for side, idx, angle in violation_data[:3]:  # Show first 3
                        print(
                            f"    {side} segment {idx}: {angle:.3f} rad ({np.degrees(angle):.1f}°)"
                        )
    ...
    def _analyze_constraints(self, path_pair, points, result):
        ...
        if left_angles:
            ...
            print(
                f"  Left path max angle: {max_left_angle:.3f} rad ({np.degrees(max_left_angle):.1f}°)"
            )
        ...
        if right_angles:
            ...
            print(
                f"  Right path max angle: {max_right_angle:.3f} rad ({np.degrees(max_right_angle):.1f}°)"
            )
        ...
        try:
            ...
            if widths:
                ...
                print(
                    f"  Width range: {min_width:.3f}m - {max_width:.3f}m (mean: {mean_width:.3f}m)"
                )
    ...
    def test_all_tracks(self, num_points=10):
        ...
        for track in self.tracks:
            try:
                ...
                results.append({"track_id": track["id"], "status": "completed"})
            except Exception as e:
                print(f"\nTrack {track['id']} failed with error: {e}")
                results.append(
                    {"track_id": track["id"], "status": "failed", "error": str(e)}
                )
