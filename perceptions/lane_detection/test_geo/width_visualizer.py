...
class TrackWidthVisualizer:
    ...
    def visualize_track_widths(self, track_id, figsize=(12, 8)):
        ...
        plt.suptitle(f'Track {track_id} - Comprehensive Width Analysis', fontsize=16, fontweight='bold')
        plt.tight_layout()
        plt.show()
    ...
    def _plot_track_layout(self, ax, track, width_analysis):
        ...
        for i, match in enumerate(width_analysis['matching_lines']):
            ...
            if width < 2.0 or width > 7.0:
                ...
                ax.text(mid_x, mid_y, f'{width:.1f}m', fontsize=8, 
    ...
    def _plot_width_vs_position(self, ax, width_analysis):
        ...
        wmin, wmax = 2.5, 6.5
        ax.fill_between(positions, wmin, wmax, alpha=0.2, color='green', label='Valid Range')
        ...
        for violation in violations:
            ax.scatter(violation['index'], violation['width'], 
                      color='red', s=100, zorder=5, alpha=0.8)
    ...
    def _plot_violation_analysis(self, ax, width_analysis):
        ...
        labels = [f'Valid ({valid})', f'Too Narrow ({too_narrow})', f'Too Wide ({too_wide})']
...
Violation Rate: {stats['violation_rate']:.1%}"""
    ...
    def visualize_all_tracks(self, figsize=(12, 8)):
        ...
        for i, track in enumerate(self.tracks):
            ...
            ax.set_title(f'Track {track["id"]} - {width_analysis["stats"]["violation_rate"]:.1%} violations')
        
        # Hide unused subplots
        for i in range(len(self.tracks), 9):
            axes[i].set_visible(False)
        
        plt.suptitle('All Tracks - Width Analysis Overview', fontsize=16, fontweight='bold')
        plt.tight_layout()
        plt.show()
    ...
    def generate_width_report(self, track_id):
        ...
        if violations:
            print(f"\nVIOLATION DETAILS:")
            for violation in violations:
                print(f"  Section {violation['index']}: {violation['type'].replace('_', ' ').title()}")
                print(f"    Width: {violation['width']:.3f}m (threshold: {violation['threshold']:.1f}m)")
