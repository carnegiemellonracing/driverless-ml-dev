...
from geo import find_matching_points, constraint_decider, bt_decider
...
class ConstraintTester:
    ...
    def test_all_constraints(self, track_id, wmin=2.5, wmax=6.5):
        ...
        constraint_results['seg_angle']['details'] = {
            'max_angle_left': max([v[2] for v in seg_violations if v[0] == 'left'], default=0),
            'max_angle_right': max([v[2] for v in seg_violations if v[0] == 'right'], default=0),
        ...
        if len(left_coords) >= 2 and len(right_coords) >= 2:
            ...
            for i in range(n):
                for j in range(i + 2, n):
                    ...
                    if self._line_segments_intersect(polygon_points[i], polygon_points[(i+1)%n],
        ...
        constraint_results['polygon']['details'] = {
            'polygon_points': len(polygon_points) if 'polygon_points' in locals() else 0,
            'intersection_count': len(poly_violations)
        }
        
        # Overall result
        constraint_results['overall_passed'] = (
            constraint_results['seg_angle']['passed'] and
            constraint_results['width']['passed'] and
    ...
    def calculate_widths_for_track(self, track_id, wmin=2.5, wmax=6.5):
        ...
        right_indices = list(range(len(track['left_points']), len(track['left_points']) + len(track['right_points'])))
        ...
        width_analysis['stats'] = {
            ...
            'violation_rate': len(width_analysis['violations']) / len(widths) if widths else 0
        }
    ...
    def visualize_constraint_test(self, track_id, figsize=(12, 8)):
        ...
        plt.suptitle(f'Track {track_id} - Complete Constraint Analysis', fontsize=16, fontweight='bold')
        plt.tight_layout()
        plt.show()
    ...
    def _plot_track_with_constraints(self, ax, track, constraint_results):
        ...
        # Plot boundaries
        ax.plot(left_points[:, 0], left_points[:, 1], 'b-', linewidth=3, label='Left Boundary', alpha=0.7)
        ax.plot(right_points[:, 0], right_points[:, 1], 'r-', linewidth=3, label='Right Boundary', alpha=0.7)
        ...
        for side, idx, angle in seg_violations:
            if side == 'left' and idx < len(left_points) - 1:
                ax.scatter(left_points[idx+1, 0], left_points[idx+1, 1], 
                          color='red', s=100, marker='X', zorder=5, label='Sharp Turn' if idx == 0 else "")
            elif side == 'right' and idx < len(right_points) - 1:
                ax.scatter(right_points[idx+1, 0], right_points[idx+1, 1], 
                          color='red', s=100, marker='X', zorder=5, label='Sharp Turn' if idx == 0 else "")
    ...
    def _plot_constraint_summary(self, ax, constraint_results):
        ...
        wedges, texts, autotexts = ax.pie([1, 1, 1], labels=constraints, colors=colors, autopct='%s', startangle=90)
    ...
    def _plot_width_distribution(self, ax, width_analysis):
        ...
        # Create histogram
        n, bins, patches = ax.hist(widths, bins=20, alpha=0.7, color='skyblue', edgecolor='black')
        ...
        ax.axvline(wmin, color='red', linestyle='--', linewidth=2, label=f'Min Width ({wmin}m)')
        ax.axvline(wmax, color='red', linestyle='--', linewidth=2, label=f'Max Width ({wmax}m)')
        ax.axvline(stats['mean'], color='blue', linestyle='-', linewidth=2, label=f'Mean ({stats["mean"]:.2f}m)')
    ...
    def _plot_polygon_analysis(self, ax, track, constraint_results):
        ...
        # Plot polygon
        polygon = plt.Polygon(polygon_points, alpha=0.3, facecolor='lightblue', edgecolor='blue')
        ax.add_patch(polygon)
        
        # Highlight intersection points
        poly_violations = constraint_results['polygon']['violations']
        for i, j in poly_violations:
            ax.scatter(polygon_points[i, 0], polygon_points[i, 1], color='red', s=50, marker='o')
            ax.scatter(polygon_points[j, 0], polygon_points[j, 1], color='red', s=50, marker='o')
    ...
    def _plot_overall_results(self, ax, constraint_results):
        ...
        ax.text(0.5, 0.7, 'OVERALL RESULT', ha='center', va='center', fontsize=16, fontweight='bold')
        
        status = "PASSED" if overall_passed else "FAILED"
        color = 'green' if overall_passed else 'red'
        ax.text(0.5, 0.5, status, ha='center', va='center', fontsize=24, fontweight='bold', color=color)
